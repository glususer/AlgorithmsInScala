...
/**
  * Created by shivangi on 15/08/16.
  */
...
class MTreeSuite extends FunSuite {
  ...
  val t1 =  MTree('a', List(MTree('f', List(MTree('g'))), MTree('c'), MTree('b', List(MTree('d'), MTree('e')))))
  println(t1.nodeCount)
  println(t1.internalPathLength)
 // println(t1.postOrder)
